...
(function(){
  ...
  function render(){
    ...
    bEl.innerHTML =
      ...
      tile("ต้นทุนรวม", fmt(bizCost, 0), "บาท/เดือน คงที่ " + fmt(fixed, 0) + " +  ADS ") +
    ...
    if (spend <= 0 || impr <= 0) {
      ...
      action = "ใส่งบที่จ่ายไปจริง และจำนวนครั้งที่ ADS ถูกแสดง (Impressions) ก่อน แล้วกดคำนวณอีกครั้ง";
    } else if (conv === 0 && target > 0 && spend >= target * 2) {
      cls = "bad"; title = "หยุด ADS ชุดนี้ก่อน";
      action = "จ่ายไปแล้ว " + fmt(spend, 0) + " บาท มากกว่าราคาที่เรารับได้ต่อลูกค้า 1 คน (" + fmt(target, 0) + " บาท) ถึงสองเท่า แต่ยังไม่มีใครสมัครเลยสักคน ยิ่งปล่อยต่อยิ่งเสียเงินเปล่า เข้าไปปิดชุด ADS นี้ใน Ads Manager";
    } else if (clicks < 50 && conv < 3) {
      cls = "warn"; title = "ยังสรุปอะไรไม่ได้ อย่าเพิ่งแตะ";
      action = "ตอนนี้มีคนกดเข้ามาแค่ " + fmt(clicks, 0) + " คน และสมัคร " + fmt(conv, 0) + " คน น้อยเกินกว่าจะบอกได้ว่า ADS ดีหรือแย่ ปล่อยให้ยิงต่อจนครบ 50 คลิก หรือมีคนสมัคร 3 คนก่อน แล้วค่อยกลับมาดู";
    } else if (days < 3) {
      cls = "warn"; title = "รออีก " + fmt(3 - days, 0) + " วัน";
      action = " ADS เพิ่งยิงมา " + fmt(days, 0) + " วัน ช่วง 3 วันแรกระบบยังทดลองหาว่าควรแสดงให้ใครดู ตัวเลขจึงยังแกว่ง ถ้าไปแก้อะไรตอนนี้ ระบบจะเริ่มเรียนรู้ใหม่หมดและต้นทุนจะพุ่ง";
      ...
      cls = "warn"; title = "เริ่มแพง ให้เปลี่ยนภาพและข้อความ ADS ";
      action = "ได้ลูกค้า 1 คนในราคา " + fmt(cpa, 0) + " บาท แพงกว่าเพดาน " + fmt(target, 0) + " บาท แต่ยังไม่ถึงขั้นต้องหยุด อย่าเพิ่งลดงบ ให้เปลี่ยนภาพหรือข้อความ ADS เป็นชุดใหม่ก่อน เพราะส่วนใหญ่ปัญหาอยู่ที่ตัว ADS ไม่ดึงดูดพอ แล้วดูผลอีก 3 วัน";
    } else if (days < 7) {
      ...
      cls = "bad"; title = "ปิด ADS ชุดนี้ แล้วย้ายงบไปชุดที่ถูกกว่า";
      action = "ได้ลูกค้า 1 คนในราคา " + fmt(cpa, 0) + " บาท ทั้งที่เรารับได้แค่ " + fmt(target, 0) + " บาท และเป็นแบบนี้ต่อเนื่องมา " + fmt(days, 0) + " วันแล้ว ทุกวันที่ปล่อยไว้คือขาดทุนเพิ่ม ให้เข้าไปปิดชุด ADS นี้ใน Ads Manager แล้วเอางบรายวันของมันไปเติมให้ชุดอื่นที่ได้ลูกค้าถูกกว่า ถ้าไม่มีชุดไหนดีเลย ให้หยุดทั้งหมดก่อน แล้วกลับไปแก้ภาพ ข้อความ และหน้าเว็บปลายทาง ค่อยเริ่มใหม่";
    }
    ...
    if (impr <= 0 || clicks <= 0) {
      ...
      diag = "สาเหตุ: คนเห็น ADS แล้วไม่กด กดแค่ " + fmt(ctr, 2) + " % จาก 100 คนที่เห็น แปลว่าภาพหรือพาดหัวยังไม่สะดุดตา ลองเปลี่ยนภาพ เปลี่ยนพาดหัว หรือทำ 3 วินาทีแรกของวิดีโอให้น่าดูกว่านี้";
    } else if (cvr < 1.5) {
      diag = "สาเหตุ: คนกดเข้าเว็บแล้วแต่ไม่สมัคร สมัครแค่ " + fmt(cvr, 2) + " % ปัญหาอยู่ที่หน้าเว็บปลายทาง ไม่ใช่ตัว ADS เช็กว่าหน้าโหลดช้าไหม สมัครกี่ขั้นตอน และบอกชัดหรือยังว่าจ่ายเท่าไรแล้วได้อะไร";
      ...
      diag = "สาเหตุ: ตัว ADS ไม่ได้มีปัญหา แต่ลูกค้า 1 คนทำเงินให้เราได้แค่ " + fmt(ltvCac, 2) + " เท่าของค่าที่จ่ายไปเพื่อได้เขามา ควรได้อย่างน้อย 3 เท่า ทางแก้คือขึ้นราคา ขายพ่วง หรือทำให้ลูกค้าอยู่กับเรานานขึ้น";
    } else {
    ...
    vEl.innerHTML =
      ...
      "<p>" + action + "</p>" + (diag ? '<p class="diag">' + diag + "</p>" : "");g
  }
